---
# Feel free to add content and custom Front Matter to this file.
# To modify the layout, see https://jekyllrb.com/docs/themes/#overriding-theme-defaults

layout: page
title: Home
---

# About me

I'm a Ph.D. candidate at [Georgia Tech](https://www.gatech.edu/) advised by Prof. [Faramarz Fekri](https://fekri.ece.gatech.edu/).
My research primarily focuses on Machine Learning, Probabilistic graphical models and Causal Discovery. In particular, I'm interested in
understanding the feasibility of recovering cyclic causal graph structure from observational and interventional data under various real-world constraints (including unmeasured confounding, missing data, and measurement noise). Previously, I got my Masters from Georgia Tech and I did my undergraduate studies at National Institute of Technology, Trichy ([NITT](https://www.gatech.edu/)). During my Ph.D. I completed an internship at [Genentech](https://www.gene.com), where I had the pleasure of working with [Jan-Christian Hütter](https://www.jchuetter.com/).
